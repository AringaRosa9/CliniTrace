...
uv run celery -A app.workers.runtime:celery worker -Q parsing,extracting,exporting --loglevel=ERROR --concurrency=2 --pool=prefork > /tmp/bljgh-s1-test-worker.log 2>&1 &
worker_pid=$!
