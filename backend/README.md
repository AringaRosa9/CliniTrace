...
S1 已实现身份/项目、患者/就诊、上传/预览、异步任务及审计接口；统一 request_id/error/no-store 中间件已就位。`app/api/s1.py` 和 `app/api/s2.py` 挂载实际文档、OCR 与抽取路由；`app/contracts/api.py` 合并 S1–S2 与未来 S3 设计契约。
