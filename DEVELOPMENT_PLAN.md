...
工程实现与本地合成验收见 [S5 验收记录](docs/testing/s5-report.md)、[运行手册](docs/operations/s5.md) 与 [UAT/培训材料](docs/operations/pilot/uat-training.md)。下列原始 REL 工单包含机构现场验收，不能以工程交付替代全部退出条件，因此尚未全部勾选。

- [x] REL-01 工程部分：补充发布门禁测试、六页面×五视口 axe 检查、键盘/200% CSS 缩放、授权 API 合成并发负载、审核工作台无障碍及原安全/并发/任务恢复回归。
- [x] REL-02 工程部分：配置/镜像/Compose/角色模板、私有监控/告警、维护模式、一致性备份恢复、项目删除凭证与重试、隔离演练、兼容回滚工具。完整镜像构建因 registry EOF 待网络可用后验证。
- [x] REL-03 工程部分：材料/hash/四方签署/16 类指标与分母/阻断缺陷/留存门禁，缺项返回不放行；UAT 执行与缺陷记录模板。
- [x] REL-04 工程部分：OIDC 试点项目/成员初始化工具、操作培训、主备值班及灰度/回滚决策材料。
...
DOC-01～06、EXT-01～06、REV-01～05 和 DATA-01～03 的本地工程实现及合成验证已完成。QLT-01～06 的管理与质量闭环工程实现及合成验证也已完成。S5 交付工程与本地合成验证已实现；下一步在获准环境完成镜像构建、机构验收与灰度，并继续推进 FND-05：获准模型服务、中文 OCR 环境、临床样本、正式词库与预算。S2 阶段的真实质量/性能验收尚未完成，不能以合成测试替代。
