...
状态：S0 工程基线已实现并通过本地验证（2026-09-16）；FND-05 的机构授权决策待提供。详见 [S0 验收记录](docs/testing/s0-report.md)。S1 文档与持久化已完成本地合成开发与验证（2026-09-17），见 [S1 验收记录](docs/testing/s1-report.md)；S2 抽取与证据工程实现已完成本地合成验证（2026-09-17），见 [S2 验收记录](docs/testing/s2-report.md)。S3–S5 尚未实施；S2 的获准真实模型/中文 OCR/临床样本验收仍待 FND-05 条件。
...
- [ ] REV-01（FE）：按原型拆解 AppShell、CaseContext、DocumentViewer、FactTable、Timeline、JsonView、IssuePanel、HistoryDrawer、ReviewFooter。
- [ ] REV-02（FE／BE）：字段↔原文双向定位、页码／缩放、多证据及重叠证据选择；加载错误有恢复入口。
- [ ] REV-03（BE／FE）：编辑、补录、排除、理由、原始／当前值对照，刷新持久化与版本冲突处理。
- [ ] REV-04（BE／FE／MED）：逐项核对、问题处置、接受未知、退回补料、最终声明与服务端审核门禁。
- [ ] REV-05（BE／QA）：快照事务、多人并发、审核后编辑失效、新运行切换与新增文书导致范围变更。
- [ ] DATA-01（BE／FE）：患者／就诊／文书／状态筛选，审核数量和候选事实统计全部来自服务端。
- [ ] DATA-02（BE／FE）：异步 JSON／CSV 导出、默认已审核、草稿权限与标记、快照冻结、用途和下载审计。
- [ ] DATA-03（QA／BE）：JSON 嵌套／证据／版本完整性；CSV 一行一事实修订、关系键、多证据可序列化字段和空值说明；中文、引号、换行、公式注入检查。

**S4：管理与质量闭环**

- [ ] QLT-01（BE／FE／MED）：模板草稿／发布／新版本、指南、正反例及证据规则；发布前 Schema 和兼容性校验。
- [ ] QLT-02（BE／FE／AI）：词库版本导入／激活、候选确认、依据和映射历史；无授权版本不能发布。
- [ ] QLT-03（AI／MED）：100～300 份起步样本的来源和难例覆盖计划、标注、二审、分歧裁决、患者隔离及冻结清单。
- [ ] QLT-04（AI／BE）：离线评测 runner、字段／span／关系／时间／术语指标、错误分层、成本与人工时间记录。
- [ ] QLT-05（FE／BE）：质量看板、版本对比、样本量／分母／数据集版本、无结果空态、权限化错误样本查看。
- [ ] QLT-06（AI／MED／BE）：纠错候选池、二次复核、拒收／接收理由、版本发布、测试集污染检测。
...
DOC-01～06 和 EXT-01～06 的本地工程实现及合成验证已完成。下一步进入 S3 工作台、审核与交付，并继续推进 FND-05：获准模型服务、中文 OCR 环境、临床样本、正式词库与预算。S2 阶段的真实质量/性能验收尚未完成，不能以合成测试替代。
